...
def actor_menu_method
  ...
  if Actor.find_by(slug: actor_slug) != nil
    ...
    puts "#{actor.split.map(&:capitalize).join(' ')} is currently starring in #{movie_titles.join(', ')}."
  else
...
def director_menu_method
  ...
  if Director.find_by(slug: director_slug) != nil
    puts "#{director.split.map(&:capitalize).join(' ')} currently has the following movies in theaters:"
